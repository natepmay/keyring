...
export class XyPoint extends Point {
  ...
  toI() {
    let x = this.x;
    let y = -this.y; // Flip axis, for SVG
    return Scalar.wrap(12 * (5 / 4 - Math.atan2(y, x) / (2 * PI)), 12)
  }
  ...
  /**
   * Place a point along a line at a linear position given by an
   * interpolationFactor between 0 and 1.
   * 
   * @param interpolationFactor
   *  0 = place point at the start of the line
   *  1 = place point at the end of the line
   */
  static alongLine(line: XyLine, interpolationFactor: number) {
    const x = CustomMath.linearInterpolate(interpolationFactor,
      {in: 0, out: line.start.x}, {in: 1, out: line.end.x}
    );
    const y = CustomMath.linearInterpolate(interpolationFactor,
      {in: 0, out: line.start.y}, {in: 1, out: line.end.y}
    );
    return new XyPoint(x, y);
  }
