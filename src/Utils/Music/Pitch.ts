...
export class Pitch {
  ...
  get midiNumber(): number {
    return Midi.noteNumberFromNoteIdAndOctave(this.note.id, this.octave);
  }

  static fromMidiNumber(n: number): Pitch {
    const { octave, noteId } = Midi.noteIdAndOctaveFromNoteNumber(n);
    return new Pitch(new Note(noteId), octave);
  }
  ...
  get frequency(): number {
    return Midi.frequencyFromNoteNumber(this.midiNumber);
  }
