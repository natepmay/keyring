...
export class Note {
  ...
  /**
   * Return a Pitch that is within one octave above the lowest note when the
   * lowest is placed within the given SPN octave.
   * 
   * @param lowestNoteId
   *  The Note ID of the lowest note in the octave
   * 
   * @param lowestNoteOctave
   *  The [SPN](https://en.wikipedia.org/wiki/Scientific_pitch_notation)
   *  octave number in which we would like to place the lowest note.
   */
  pitchAboveLowestNoteInOctave(
    lowestNoteId: number,
    lowestNoteOctave: number
  ) {
    const octave = lowestNoteOctave + (this.id < lowestNoteId ? 1 : 0);
    return new Pitch(this, octave);
  }
