...
export class AppState {
  ...
  constructor(props: AppStateProps) {
    ...
    const addStructure = (intervalSetBinary: number) =>
      this.collection.add({
        intervalSetBinary,
        activeLayerStore: this.activeLayer,
      });
  ...
  get targetLayers(): Readable<Layer[]> {
    return derived(
      [this.editingMode, this.activeLayer, this.checkedLayers],
  ...
  get playStructure() {
    ...
    return (structure: Structure) => {
      ...
      const effectiveIntervalSet = get(structure.intervalSet).shift(
        -lowestNoteOrdinal
      );
