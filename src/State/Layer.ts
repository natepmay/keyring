...
function unifyLayerDatum(
  ...
  return derived(
    mapStores(layerDataArray, selector),
    values => values.reduce((a, b) => a && b, true),
  );
}
...
function unifyAsyncLayerActions(a: AsyncLayerAction, b: AsyncLayerAction): AsyncLayerAction {
  return derived([a, b], ([$a, $b]) => {
    if ($a === undefined || $b === undefined) { return undefined; }
    return async () => { $a(); $b(); }
  });
